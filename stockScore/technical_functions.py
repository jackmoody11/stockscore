...
def moving_avg_test(batch_symbols, stock_scores, api_url_base=iex_url_base):
    ...
    for i in batch_symbols:
        batch_url = api_url_base + "stock/market/batch?symbols=" + \
        ...
        for symbol in result:
            ...
            if base['day200MovingAvg'] and base['day50MovingAvg']:
                ...
                if 0 < per_diff < 2:
                    ...
                    print(symbol + " score went up by " + str(round(5 / (score + 1))
                                                              ) + " -- SMA 200 under SMA 50 by " + str(per_diff) + "%")
                elif 2 < per_diff < 5:
                    stock_scores[symbol] += round(5 / score)
                    print(symbol + " score went up by " + str(round(5 / score)
                                                              ) + " -- SMA 200 under SMA 50 by " + str(per_diff) + "%")
