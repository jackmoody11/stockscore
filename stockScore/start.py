...
def get_symbols(url_base=iex_url_base, manual=False):
    ...
    if last_updated is None or time_diff > datetime.timedelta(days=5) or manual:

        symbols_json = requests.get(url_base + "ref-data/symbols").json()
        symbols = []
...
def xl_get_stats():
    ...
    for symbol in symbols:
        for characteristic in characteristics:
            column = characteristics.index(characteristic) + 2
            if ws.cell(column=column, row=3).value == characteristic:
                for i in range(4, 6000):
                    if ws.cell(column=column, row=i).value is None:
...
def get_stats(iex_url_base=iex_url_base, manual=False):
    ...
    if last_updated is None or time_diff > datetime.timedelta(days=5) or manual:
        for i in batch_symbols:
            batch_url = iex_url_base + "stock/market/batch?symbols=" + \
        ...
        for symbol in symbols:
            for characteristic in characteristics:
                column = xl.utils.cell.get_column_letter(
                    characteristics.index(characteristic) + 2)
                row = symbols.index(symbol) + 4
                cell_name = str(column) + str(row)
                ws[cell_name].value = stats[symbol][characteristic]
                print("success")
...
def return_top(dictionary, x=None):
    ...
    if x is None:
        x = len(dictionary)
    sorted_array = sorted(dictionary.items(), key=lambda x: x[1], reverse=True)
